...
export async function POST(req: NextRequest) {
  try {
    // Already auth-gated since the SSR migration. Switching to the shared
    // requireUser() helper from lib/auth.ts for consistency. Behavior
    // unchanged: validates the JWT via getUser() (server-side, not just
    ...
    if (!landlordId || landlordId !== auth.userId) {
